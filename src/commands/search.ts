...
import { hasAdminPermission, sendPermissionDenied, getSubcommand, getStringOption } from '../handlers/interactionCreate.js';
...
export async function handleSearchCommand(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    switch (subcommand) {
      ...
      default:
        await interaction.reply({
          content: '❌ Unknown subcommand. Available: `toggle`, `quota`, `test`.',
          ephemeral: true,
        });
    }
...
async function handleToggleSubcommand(
  ...
  if (enabled) {
    ...
    const remaining = Math.max(0, 2000 - usage); // Brave Search free tier: 2000 queries/month
    statusMessage += `\n\n📊 **Current Usage:** ${usage}/2000 queries this month (${remaining} remaining)`;
  }
...
async function handleQuotaSubcommand(
  ...
  try {
    ...
    const maxQueries = 2000; // Brave Search free tier limit
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(usagePercentage > 90 ? 0xff0000 : usagePercentage > 70 ? 0xffa500 : 0x00ff00)
      .setTimestamp()
    ...
    const apiKeyStatus = process.env.BRAVE_SEARCH_API_KEY ? '✅ Configured' : '❌ Missing';
    
    embed.addFields({
      name: '⚙️ Search Status',
      value: [
        `**Server Setting:** ${statusIcon} ${searchEnabled ? 'Enabled' : 'Disabled'}`,
    ...
    if (usagePercentage > 90) {
      recommendations = '⚠️ **Warning:** Approaching monthly limit. Consider reducing search usage.';
    } else if (usagePercentage > 70) {
      recommendations = '🟡 **Caution:** High usage detected. Monitor search activity.';
    ...
    await interaction.editReply({
      content: '❌ Failed to retrieve search quota information. Please try again later.',
    });
  }
}
...
async function handleTestSubcommand(
  ...
  try {
    ...
    if (!searchEnabled) {
      await interaction.editReply({
        content: '❌ Web search is disabled for this server. Enable it with `/search toggle enable`.',
      });
      return;
    }
    ...
    if (!process.env.BRAVE_SEARCH_API_KEY) {
      await interaction.editReply({
        content: '❌ Brave Search API key is not configured. Cannot perform test search.',
      });
      return;
    }
    ...
    if (currentUsage >= 2000) {
      await interaction.editReply({
        content: '❌ Monthly search quota exceeded. Cannot perform test search.',
      });
      return;
    }
    ...
    await interaction.editReply({
      content: '❌ Search test failed. Please check the configuration and try again.',
    });
  }
}
...
async function simulateSearchTest(
  ...
  embed.addFields({
    name: '⚠️ Note',
    value: 'This is a simulated test. Actual search integration is pending Phase 3 completion.',
    inline: false,
  });
