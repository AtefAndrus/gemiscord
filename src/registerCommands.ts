...
if (!token || !clientId) {
  logger.error('Missing required environment variables: DISCORD_TOKEN, DISCORD_CLIENT_ID');
  process.exit(1);
}
...
async function registerCommands(): Promise<void> {
  try {
    logger.info(`Starting registration of ${commands.length} slash commands...`);
    ...
    logger.info(`Successfully registered ${data.length} slash commands (${registrationType})`);
    ...
    if (!guildId) {
      logger.warn('Global commands may take up to 1 hour to propagate across Discord');
    }
