...
describe('Slash Commands Integration Tests', () => {
  ...
  const testPrefix = `slash-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  beforeAll(async () => {
    ...
    configService = new ConfigService('sqlite://tests/fixtures/test-slash-commands.sqlite');
  ...
  describe('Command Registration Integration', () => {
    it('should have proper command structure for Discord API', () => {
      ...
      const expectedCommands = [
        ...
        {
          name: 'search',
          description: 'Manage web search functionality and monitor quota usage',
          defaultMemberPermissions: '8',
          options: [
            { name: 'toggle', type: 1 },
            { name: 'quota', type: 1 },
        ...
        {
          name: 'model',
          description: 'View AI model information, usage statistics, and rate limits',
          defaultMemberPermissions: '8',
          options: [
            { name: 'info', type: 1 },
            { name: 'stats', type: 1 },
      ...
      expect(expectedCommands.every(cmd => cmd.defaultMemberPermissions === '8')).toBe(true);
      expect(expectedCommands.every(cmd => cmd.name && cmd.description)).toBe(true);
    });
  });
  ...
  describe('Config Command Integration', () => {
    ...
    it('should manage server prompts end-to-end', async () => {
      const testPrompt = 'This is a test server prompt for integration testing.';
  ...
  describe('Error Recovery Integration', () => {
    it('should handle database connection errors gracefully', async () => {
      ...
      const invalidConfigService = new ConfigService('invalid://not-sqlite-protocol');
    ...
    it('should handle missing environment variables', () => {
      ...
      try {
        ...
        if (originalSearchKey) process.env.BRAVE_SEARCH_API_KEY = originalSearchKey;
      }
    });
  });
});